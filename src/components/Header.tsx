...
function Header() {
  ...
  return (
    <div className="bg-[#121212] fixed top-0 left-0 w-full z-50">
      ...
      {/* container  */}
     <main className="md:px-[104px] px-[20px] py-[20px] flex items-center justify-between bg-[#121212] z-50 relative">
       {/* logo */}
      <section className=" ">{HeaderIcon.logo}</section>
      {/* navigation  */}
      <section className="hidden md:flex items-center gap-[24px] text-white text-[16px] font-[500]">
      ...
      {/* connect wallet  */}
      <button className=" px-[32px] py-[10px] text-[16px] font-[] rounded-[8px] bg-gradient-to-b from-[#1F51FF] to-[#8A2BE2] text-white cursor-pointer hover:scale-[105%] transition-all duration-300 hidden md:inline-block">Connect Wallet</button>
      <button className="  text-white text-2xl cursor-pointer hover:scale-[105%] md:hidden inline-block" onClick={()=>setOpenMenu(!openMenu)}>{openMenu ? <MdClose/>:<FaBars/>}</button>
